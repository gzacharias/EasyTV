...
// This leaves passphrase_key empty if failed to get a valid key
...
async function tmdb_get(path) {
  ...
  const response = await fetch(`${TMDB_API_BASE}${path}${separator}api_key=${tmdb_api_key}`);
...
function render_show_row(item) {
  ...
  row_div._refresh_header = async () => {
    try {
      const progress = await trakt.get_show_progress(show.ids.trakt);
...
function render_season_row(season, show_id, auto_open = false) {
  ...
  details.addEventListener("toggle", async () => {
    ...
    try {
      const episode_details = await trakt.get_season_episodes(show_id, season.number);
...
function render_episode_row(episode, auto_open = false) {
  ...
  checkbox.addEventListener("change", async () => {
    ...
    try {
      if (checkbox.checked) {
        await trakt.mark_episode_watched(episode.ids.trakt);
      } else {
        await trakt.mark_episode_unwatched(episode.ids.trakt);
      }
...
async function run_device_login() {
  ...
  try {
    const login_data = await trakt.get_login_data();

    await show_code_dialog(login_data.user_code, login_data.verification_url);
...
async function load_watchlist() {
  ...
  try {
    const [profile, watchlist_items, watched_items] = await Promise.all([
      trakt.get_profile(),
      trakt.get_watchlist(),
      trakt.get_watched_shows(),
    ]);

    const hidden_items = await trakt.get_hidden_shows()
...
(async () => {
  // We rely to trakt.init doing some decryption and hence validating the passphrase.
  // If it ever stops doing that, we will need to find some other way to validate it.
  await trakt.init();
